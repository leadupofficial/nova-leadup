...
| 4b | In-app notification surface | The user can read and clear nudges | `/notifications` exists (list, read, delete, unread-count) and **the app still calls none of it**. The bell shows a hardcoded `0`. | Never wired | **P1** | OPEN | Call the existing endpoints from a notifications sheet; the badge should be the real unread count | A user who dismisses the shade cannot find the nudge again inside NOVA |
