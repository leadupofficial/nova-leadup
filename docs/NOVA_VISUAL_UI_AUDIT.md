...
| **Admin console** | Correct and unusually candid: *"The admin console is limited to the owner and admin roles. This session is signed in as \"user\", and every /api/v1/admin route would answer 403."* One layout defect, now **fixed**: the app-bar title was truncated to **"Admin C…"**. |
| **Offline** | Reached by turning connectivity off — `adb shell cmd connectivity airplane-mode enable`, confirmed with a failed ping; the `/offline` deep link cannot reach it while online, and correctly rendered Home instead. Clean centred layout, no orb (right for a state where voice cannot work), and specific copy. One wording problem, now **fixed**: the headline said *"Your saved reminders still work"* while the table beneath read **"Reminders — Not cached"**. |
...
## 13. Two P3s fixed and verified (2026-09-23)

| Screen | Issue | Sev | Root cause | Fix | Verification |
|---|---|---|---|---|---|
| Admin console | App-bar title truncated to **"Admin C…"** | P3 | The bar reproduced CSS `justify-content: space-between` with **two `Spacer`s plus a `Flexible` title**. The spacers are `Expanded`, so they took two thirds of the free space and left the title a third — enough for "Admin C…" and no more. | `mainAxisAlignment: MainAxisAlignment.spaceBetween` with the three children and no spacers — which is what `space-between` actually does: it distributes the *gaps*, so the title keeps its natural width. | `r61_admin.png` — the bar reads **"Admin Console"** in full, pill still at the right edge |
| Offline | Headline contradicted the table beneath it | P3 | *"Your saved reminders still work"* sat directly above **"Reminders — Not cached"**. Both are true of different things — an armed alarm is scheduled on the device and fires offline (verified with the radio off), while the *list* is not cached for browsing — but the screen never said which. | *"Reminders you have already set still fire."* The widget test that pinned the old sentence now asserts the reassurance rather than the phrasing, and carries the reason. | `r61_offline.png`, captured with airplane mode on and confirmed by a failed ping |

## 14. Translate — action buttons stacked one per row (2026-09-23)

| Screen | Screenshot | Issue | Sev | Root cause | Fix | Verification |
|---|---|---|---|---|---|---|
| Translate | `r59_translate.png` | The action buttons stacked **one per line** — `Speak` above `Paste`, and `Copy` / `Edit` / `Speak` / `Share` in four rows — consuming about four lines of vertical space per card and pushing the page down. | **P3** | The actions sit in a `Wrap`, which lays children out side by side — but each child was built as `SizedBox(height: 44, child: Center(child: …))`, and **a bare `Center` expands to fill the width it is given**. Each button therefore claimed a whole line, so the `Wrap` had nothing left to place beside it. | `Center(widthFactor: 1)`, which sizes the centre to the button instead of the available width. The 44 px hit height is untouched. | `r62_translate.png` — `Speak \| Paste` on one row, `Copy \| Edit \| Speak \| Share` on one row, and the space they were wasting is returned to the page |

A scan of every other `Wrap` in `lib/` found no second instance of the pattern.
