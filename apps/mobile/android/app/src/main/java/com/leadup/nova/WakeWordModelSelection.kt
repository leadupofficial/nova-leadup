...
/**
 * Which installed wake word the service listens for.
 *
 ...
 *
 * The engine is given exactly one wake word, so this returns a single name. With
 * one wake word installed the answer is always that one, which is why the
...
object WakeWordModelSelection {
     ...
     * @param installed the wake word names whose assets are actually present.
