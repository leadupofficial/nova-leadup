...
import { hashToken, compareTokenHash } from '../crypto.js';
import { q, qOne, closePool } from './db.js';
...
export async function findSessionByToken(token: string): Promise<SessionRow | null> {
	...
	const { rows } = await q<SessionRow>(
		'SELECT id, user_id, organization_id, workspace_id, token_hash, refresh_token_hash, status, mfa_verified, expires_at, created_at FROM sessions WHERE status = $1 AND expires_at > now() AND token_hash = $2 LIMIT 1',
		['active', tokenHash]
	);
	return rows[0] ?? null;
}
...
export async function findSessionByRefreshToken(refreshToken: string): Promise<SessionRow | null> {
	...
	const { rows } = await q<SessionRow>(
		'SELECT id, user_id, organization_id, workspace_id, token_hash, refresh_token_hash, status, mfa_verified, expires_at, created_at FROM sessions WHERE status = $1 AND expires_at > now() AND refresh_token_hash = $2 LIMIT 1',
		['active', refreshTokenHash]
	);
	return rows[0] ?? null;
}
